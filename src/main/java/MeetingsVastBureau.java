...
public class MeetingsVastBureau {


    WebDriverWait wait;
    WebDriver driver;

    public MeetingsVastBureau(WebDriver driver) {
        this.driver = driver;
    ...
    @FindBy(xpath = "//div[@id='Besluit_BesluitInhoudVastbureau']")
    private WebElement vastBureauText;

    @FindBy(xpath = "//nav[@class='main-nav']//a[contains(text(),'Besluiten')]")
    private WebElement goToBesluiten;

    public MeetingsGemeenteraad meetingVastBureau() {
        ...
        return new MeetingsGemeenteraad(this.driver);
    }
    ...
    public boolean checkTextInAgendapunkt() {
        ...
        if (
                (vastBureauText.getText().contains("Vast Bureau"))/* && (samenvattingText.getText().contains("SAMENVATTING")) &&
