...
public class MeetingsBCSD {


    WebDriverWait wait;
    WebDriver driver;

    public MeetingsBCSD(WebDriver driver) {
        this.driver = driver;
    ...
    @FindBy(xpath = "//a[contains(text(),'27/12/2018 00:00')]")
    ...
    @FindBy(xpath = "//div[@id='Besluit_BesluitInhoudBcsd']")
    private WebElement bcsdText;

    @FindBy(xpath = "//nav[@class='main-nav']//a[contains(text(),'Besluiten')]")
    private WebElement goToBesluiten;

    public MeetingsAGBMdir meetingBCSD() {
        ...
        return new MeetingsAGBMdir(this.driver);
    }
    ...
    public boolean checkTextInAgendapunkt() {
        ...
        if
        (bcsdText.getText().contains("BCSD")) {
            return true;
        }
        return false;
